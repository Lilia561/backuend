...
function GoalPlanner() {
  ...
  const strokeDashoffset = circumference - (progressPercentage / 100) * circumference; // Use progressPercentage
  ...
  return (
    <div className={styles.goalPlannerCard}>
      ...
      <div className={styles.goalCircleContainer}>
        ...
        >
          ...
          <text
            ...
            className={styles.goalPercentage} /* Using the class name from GoalPlanner.module.css */
          >
            {Math.round(progressPercentage)}% {/* Display rounded percentage */}
          </text>
        </svg>
      </div>
      ...
      <div className={styles.goalDetail}>
        ...
        <div className={styles.goalDetailList}> {/* Added div for scrollable list */}
          {goals && goals.length > 0 ? (
            goals.map((item) => (
              <div className={styles.goalDetailItem} key={item.id}> {/* Use item.id for key */}
                <span>{item.purpose}</span> {/* Use item.purpose */}
                <span>
                  ₱
